...
def ai_demo_analyze_api():
    ...
    current_app.logger.info(
        ...
        stored_doc_id,
        source_filename,
        getattr(document, 'blob_name', None),
        org_id,
        reuse_last,
        doc_type,
    )
    ...
    if cached_doc_text:
        current_app.logger.info(
            'AI demo analyze using cached extracted_text for stored_doc_id=%s filename=%s',
            stored_doc_id,
            source_filename,
        )
        doc_text = cached_doc_text
        ...
        if not result.get('success') or not result.get('data'):
            current_app.logger.warning(
                'AI demo analyze failed to download stored document: stored_doc_id=%s filename=%s blob_name=%s error=%s code=%s',
                stored_doc_id,
                source_filename,
                document.blob_name,
                result.get('error'),
        ...
        if extraction_error:
            current_app.logger.warning(
                'AI demo analyze extraction failed: stored_doc_id=%s filename=%s error=%s',
                stored_doc_id,
                source_filename,
                extraction_error,
            )
...
def _log_ai_demo_unhandled_error(error):
    ...
    current_app.logger.exception('Unhandled error in ai-demo route: %s', error)
    raise error
